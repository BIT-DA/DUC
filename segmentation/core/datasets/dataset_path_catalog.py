...
class DatasetCatalog(object):
    ...
    def initMask(cfg):
        ...
        for fname in tqdm(content):
            ...
            for i in range(len(cfg.ACTIVE.SELECT_ITER)):
                mkdir_path(mask_dir.replace('gtMask/train/', 'gtMask/active_round_{}/'.format(i + 1)))
                mkdir_path(mask_dir.replace('gtMask/train/', 'gtMask/pred_round_{}/'.format(i + 1)))
                mkdir_path(mask_dir.replace('gtMask/train/', 'gtMask/label_round_{}/'.format(i + 1)))
    ...
    def initScoreMapDirs(cfg):
        ...
        for fname in tqdm(content):
            name = fname.strip()
            mask_dir = os.path.join("%s/score_maps/score/%s" % (cfg.OUTPUT_DIR, name.split("/")[0]))

            # mkdir
            mkdir_path(mask_dir)
            mkdir_path(mask_dir.replace('score/', 'purity/'))
            mkdir_path(mask_dir.replace('score/', 'entropy/'))
            mkdir_path(mask_dir.replace('score/', 'point_entropy/'))
