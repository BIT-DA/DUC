...
class DepthwiseSeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, bias=False, norm_layer=None):
    ...
    def __init__(self, inplanes, dilation_series, padding_series, num_classes, norm_layer):
        ...
        for idx, dilation in enumerate(dilation_series):
            if dilation == 1:
                ...
                branch = DepthwiseSeparableConv2d(inplanes, out_channels, kernel_size=3, stride=1, padding=dilation, dilation=dilation, bias=False, norm_layer=norm_layer)
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(out_channels * (len(dilation_series) + 1), out_channels, kernel_size=3, stride=1, padding=1, bias=False),
            norm_layer(out_channels),
            nn.ReLU(inplace=True)
        )
        ...
        self.decoder = nn.Sequential(
            DepthwiseSeparableConv2d(decoder_inplanes, decoder_out_channels, kernel_size=3, stride=1, padding=1, bias=False, norm_layer=norm_layer),
            DepthwiseSeparableConv2d(decoder_out_channels, decoder_out_channels, kernel_size=3, stride=1, padding=1, bias=False, norm_layer=norm_layer),
            nn.Dropout2d(0.1),
